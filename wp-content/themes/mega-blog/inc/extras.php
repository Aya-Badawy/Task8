 ...
 * @return array
 */
function mega_blog_body_classes( $classes ) {
	...
	// Sticky Header Class
	$classes[] = ( true === $options['menu_sticky'] ) ? 'sticky-header' : '';
	...
	if ( is_singular() || is_home() ) {
		$id = ( is_home() && ! is_front_page() ) ? get_option( 'page_for_posts' ) : get_the_id();
	  	$sidebar = get_post_meta( $id, 'mega-blog-selected-sidebar', true );
	  	$sidebar = ! empty( $sidebar ) ? $sidebar : 'sidebar-1';
	}

	if ( false === $options['message_section_enable'] ) {
		$classes[] = 'message-section-disabled';
	}
...
add_filter( 'body_class', 'mega_blog_body_classes' );
